...
class RegisterEmailManager
{
    ...
    public function sendTo(string $to, string $captcha) : void
    {
        ...
        $message->setSubject(\Parameter::instance()->get('SYSTEM_NAME').'账号申请验证邮件[请确认]');
        ...
        $message->setBody("
            ...
            <html lang='zh-cn'>
                <head>
                    <title>" . \Parameter::instance()->get('SYSTEM_NAME') . "账号申请验证邮件[请确认]</title>
                </head>
                <body>
                    <h4>您好:</h4>
                    <div>欢迎您使用" . \Parameter::instance()->get('SYSTEM_NAME'). ", 您的账号注册验证码为:{$captcha}。</div>
