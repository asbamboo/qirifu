...
abstract class ResponseAbstract implements ResponseInterface
{
     ...
     *
     * {@inheritDoc}
     * @see \asbamboo\qirifu\common\alipay\response\ResponseInterface::get()
     */
    ...
    protected function checkResponse(string $json, $decoded_json) : void
    {
        ...
        if(empty($decoded_json)){
            throw new SystemException(sprintf('支付宝返回的响应结果异常[%s]', $json));
        }

        if(!isset($decoded_json['sign'])){
            throw new SystemException(sprintf('支付宝返回的响应结果异常,sign不存在[%s]', $json));
        }
        ...
        if($sign_source_root_index === false){
            throw new SystemException(sprintf('支付宝返回的响应结果异常,response node不存在[%s]', $json));
        }
        if($sign_source_end_index === false){
            throw new SystemException(sprintf('支付宝返回的响应结果异常,sign node不存在[%s]', $json));
        }
        ...
        if($this->verifySign($sign_source, $decoded_json['sign']) != 1){
            throw new SystemException(sprintf('支付宝返回的响应结果异常,sign错误[%s]', $json));
        }
    ...
    {
        $public_pem     = \Parameter::instance()->get('ALIPAY_RSA_ALIPAY_KEY');
